...
$$LOOP(LENS)
$$IF(INDEX,0)
dbLoadRecords("db/lens.db", "LOCATION=$$LOCATION,LENSID=$$CALC{INDEX+1,%02d},NLENS=$$NLENS,RADIUS=$$RADIUS,Z=$$IF(Z,$$Z,0),Z_MOT=$$IF(Z_MOT,$$Z_MOT,0),OBJECT=$$LOCATION:LENS:BEAM:SOURCE")
$$ELSE(INDEX)
dbLoadRecords("db/lens.db", "LOCATION=$$LOCATION,LENSID=$$CALC{INDEX+1,%02d},NLENS=$$NLENS,RADIUS=$$RADIUS,Z=$$IF(Z,$$Z,0),Z_MOT=$$IF(Z_MOT,$$Z_MOT,0),OBJECT=$$LOCATION:LENS:$$LENSID$$CALC{INDEX,%02d}:IMAGE")
$$ENDIF(INDEX)
$$IF(Y_MOT)
...
$$ENDIF(Y_MOT)
$$ENDLOOP(LENS)
dbLoadRecords("db/beam.db", "LOCATION=$$LOCATION,HIGH_LIMIT=$$BEAMHIGH_LIMIT0,LOW_LIMIT=$$BEAMLOW_LIMIT0,FOCUS=$$LOCATION:LENS:$$CALC{$$COUNT(LENS),%02d}:IMAGE,SOURCE=$$BEAMSOURCE0")
$$LOOP(STACK)
$$IF(NLENS,3)
